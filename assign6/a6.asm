...
main:	stp fp, lr, [sp, alloc]!	//allocate memory
	...
	mov argc, w0	//# of arguments
	mov argv, x1	//argument array
...
next:	
	ldr x0, =openfile		
	ldr x1, [argv, 8]	//print opening file string with filename 
	bl printf
 
	mov w0, -100		//first arg
	ldr x1, [argv, 8]	//filename
	mov w2, 0		//read only
	mov w3,0		//not used
	mov x8, 56		//Openat I/O request
	svc 0			//call system function
	mov fd, w0		//file descriptor
	
	cmp fd, 0		//error check file, error if fd = -1
	b.ge next1

	ldr x0, =not_found_err		
	ldr x1, [argv, 8]	//print not found error with filename
	bl printf
	b done	
...
next1:
	...
	add buf_b, x29, buf_s	//calculate base address for the buffer
...
open_ok:
	...
	mov n_read, x0		//n_read
	
	cmp n_read, buf_size	//check the numeber of bytes read is equal to 8
	b.ne	end_of_file

	ldr x0, =x_str
	ldr d0, [buf_b]		//print x
	bl printf
...
end_of_file:
	...
	ldp fp, lr, [sp], dealloc	//deallocate memmory
	ret				//return



sin: 	stp fp, lr, [sp, -16]!	//allocate memory
	mov fp, sp
...
sinend:
	...
	ldp fp, lr, [sp], 16	//deallocate memmory
	ret			//return




cos:    stp fp, lr, [sp, -16]!	//allocate memory
        mov fp, sp
