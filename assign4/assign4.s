...
main:
		...
		add x19, fp, first_s					//calculate base address of first cuboid
		add x20, fp, second_s					//calculate base address of second cuboid

		mov x8, x19						//pass base address of first into x8
		bl newCuboid							//branch to newCuboid()
gdb1:

		mov x8, x20						//pass base address of second into x8
                bl newCuboid							//branch to newCuboid
gdb2:           		
		...
		mov x1, x19						//pass in base address for first cuboid
		bl printCuboid							//branch to printCuboid()

		ldr x0, =second_str						//load second_str
                mov x1, x20						//pass in base address for second cuboid
                bl printCuboid							//branch to printCuboid
		
		mov x0, x19						//pass base address of first cuboid
		mov x1, x20						//pass base address of second cuboid
		bl equalSize							//branch to eqaulSize()

		cmp w0, True							//result == True?
		b.ne endif1							//if not equal skip to endif
if:
		mov x0, x19						//pass in base address for first cuboid
		mov w1, 3							//pass in 3 for deltaX
		mov w2, -6							//pass in -6 for deltaY
		bl move								//branch to move()
gdb3:
		mov x0, x20						//pass in base adress for second
